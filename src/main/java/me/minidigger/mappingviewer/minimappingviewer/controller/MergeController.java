...
public class MergeController {
    ...
    public String serverMerge(@PathVariable String version, @PathVariable MappingProvider type1, @PathVariable MappingProvider type2) {
        return String.join("\n", service.merge(version, type1, type2, true));
    }

    @ResponseBody
    @GetMapping(value = "/{version}/client/{type1}/{type2}", produces = MediaType.TEXT_PLAIN_VALUE)
    public String clientMerge(@PathVariable String version, @PathVariable MappingProvider type1, @PathVariable MappingProvider type2) {
        return String.join("\n", service.merge(version, type1, type2, false));
    }
}
